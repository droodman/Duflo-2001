* dependencies: ivreg2, xtivreg2, ranktest, weakivtest, ftools, reghdfe, boottest, estout, coefplot, blindschemes, palettes, colrspace, moremata, cmp, cic, and qrprocess
...
***
{
  ...
  graph export Public\Output\targeting.png, replace width(2680) height(1552)
  ...
  * post-1995 data
  ...
  cap noi {
    * SUPAS 2005
    odbc load, clear dsn("Duflo 2001") exec("SELECT * FROM dbo.[IPUMS-based dataset] where year=2005 and Male=1 and birthprov<>96")
  ...
  //   replace yeduc = yeduc - 1 if EDUCATT==57 & occ==13  // 1 less year for Diploma I/II in teaching; go by occupation for lack of study field in 2005
    ...
    keep year relate respl urban yeduc birthyr wt indgen occ birthpl classwk birthlat birthlong ch71new en71new totinnew ninnew wsppc
    compress
    save SUPAS05, replace

    * SAKERNAS
    odbc load, clear dsn("Duflo 2001") exec("select * from [SAKERNAS 2010 dataset]")

    replace wage = wage * .194  // adjust for inflation https://data.worldbank.org/indicator/FP.CPI.TOTL?end=2010&locations=US%E2%89%A4%2FSEURLD-ID&start=1995 (immaterial in logs with year FE)
    ...
    keep year yeduc birthyr wt birthpl lwage lhwage birthlat birthlong ch71new en71new totinnew ninnew wsppc  // "birth" fields are actually for residence
    compress
    save SAKERNAS10, replace
    
    * SUSENAS
    odbc load, clear dsn("Duflo 2001") exec("select * from [SUSENAS dataset]")
    gen byte urban = 2 - B1R5
    gen int respl = B1R1 * 100 + B1R2

    mat cpi = 1.054, 1.099, 1.169, 1.244  // adjust for inflation, 2011/14->1995 https://data.worldbank.org/indicator/FP.CPI.TOTL?end=2013&locations=US%E2%89%A4%2FSEURLD-ID&start=1995
    replace B5R29 = . if B5R29 < 20000  // SUSENAS 2012 alone appears to trim observations below 20000; seems to good to do
    replace B5R29 = B5R29 * (.194 / cpi[1,year-2010])

    gen hwage = B5R29 / 4 / B5R28B if inlist(B5R31,4,5)  // if clause in response to DJL; they also count B5R31=6, affecting a few obs
    gen lwage = ln(B5R29) if inlist(B5R31,4,5)
    gen lhwage = ln(hwage)
    ren B5R31 classwk
    ...
    replace yeduc = yeduc + cond(B5R16<8, B5R16, completionyears[1, B5R15]) if B5R15>0  // DJL fix: was "cond(B5R15<8, B5R15,..." 
 
    keep year relate yeduc birthyr wt birthpl respl urban classwk lwage lhwage birthlat birthlong ch71new en71new totinnew ninnew wsppc
    save SUSENAS1114, replace
  }
  ...
  * 1995 data
  ...
  if "$source95"=="IPUMS" {
    odbc load, clear dsn("Duflo 2001") exec("select * from [IPUMS-based dataset] where Male=1 and YEAR=1995")
    ...
    if "$source95" == "NBER" {
      ...
      gen int respl = prop * 100 + kab  // regency of residence
  ...
  keep year relate urban yeduc birthyr wt indgen occ birthpl respl classwk lwage lhwage ch71new en71new totinnew ninnew wsppc recp ch71 nin en71 dens71 pop71new moldyed
  compress
  append using SUPAS05 SAKERNAS10 SUSENAS1114
  ...
  gen byte poor = inlist(floor(birthpl/100), 33, 34, 35, 53, 72, 73, 74, 81, 82) & mod(birthpl, 100) < 70  // Poor dummy, Duflo (2001), table 6, note b
  gen double lch71new = ln(ch71new)
  ...
  gen double _lwage = cond(year==1995, lhwage, cond(year==2005, IS, cond(year==2010, lhwage, lwage)))  // preferred wage var from each survey

  label var yeduc "Years of schooling"
  label var primary "Primary school completion"
  label var part "Formal employment"
  label var lwage "Log monthly wages"
  label var lhwage "Log wage"
...
collapse ninnew ch71new en71new year part yeduc primary lhwage lwage IS (rawsum) wt [aw=wt] if young<., by(young birthplnew _year)
gen _lwage = cond(inlist(year,1995,2010), lhwage, cond(year==2005, IS, lwage))
reshape wide ninnew ch71new en71new year part yeduc primary lhwage lwage IS _lwage wt, i(birthplnew young) j(_year)
...
preserve
local age74max 29
keep if age74<=`age74max'
gen t1 = (`age74max' - age74 )/8 // spline components -1974 + birth year
gen t2 = (max(0, 12 - age74))/8  // 12-year-olds in '74 couldn't benefit

forvalues y=1/1 {
  local years: word `y' of 1995 2005 2010 2011,2012,2013,2014
  local depvars : word `y' of "primary yeduc part lhwage" "primary yeduc part IS" "primary yeduc part lhwage" "primary yeduc part lwage"
  local Ndepvars: word count `depvars'
  local age74min: word `y' of 2 -8 -8 -8 -8

  forvalues c=1/1 /*0/3*/ {  // control sets, from none to full
    local ests
    local controls: word `=`c'+1' of "" lch71new "lch71new en71new" "lch71new en71new wsppc"
    forvalues d=1/`Ndepvars' {
      ...
      foreach wt in 1 wt {
        ...
        eststo RF`depvar'c`c'w`wt'y`y': reghdfe `depvar' 1.young#c.ninnew `wtexp' if young<. & inlist(year,`years'), a(birthplnew birthyr##c.(`controls')) cluster(birthplnew)
        local ests `ests' RF`depvar'c`c'w`wt'y`y'
      }
    }
    esttab `ests' using Public\Output\RF.rtf, append se nocons nostar nolines nonotes noeqlines varwidth(22) nonumber nogap msign("–") stat(N, fmt(%~1s)) fonttbl(\f0\fnil Cambria;)
  }

  * spline-based version, minimal control set
  local ests
  forvalues d=1/`Ndepvars' {
    local depvar: word `d' of `depvars'
    foreach wt in 1 wt {
      local wtexp = cond("`wt'"=="1", "", "[pw=wt]")
      eststo RF`depvar'c`c'w`wt'y`y'pwlspline: reghdfe `depvar' c.t?#c.ninnew `wtexp' if age74>=`age74min' & inlist(year,`years'), a(birthplnew##c.lch71new birthyr) cluster(birthplnew)
      local ests `ests' RF`depvar'c`c'w`wt'y`y'pwlspline
    }
  }
  esttab `ests' using Public\Output\RF.rtf, append se nocons nostar nolines nonotes noeqlines varwidth(22) nonumber nogap msign("–") stat(N, fmt(%~1s)) fonttbl(\f0\fnil Cambria;)
}
...
graph export Public\Output\Mincer4-4.png, replace width(2680) height(1552)
...
graph export Public\Output\gradlhwageyeduc1995.png, replace width(2680) height(1552)

eststo WSDlhwage95E: areg lhwage c.yeduc##c.age74 i.birthyr##c.lch71new [aw=wt] if year==1995 & young<., cluster(birthplnew) a(birthplnew)  // was "ch71"
eststo WSDlhwage95P: areg lhwage c.yeduc##c.age74 i.birthyr##c.lch71new [aw=wt] if year==1995 & (old | reallyold), cluster(birthplnew) a(birthplnew)
eststo WSDIS05     : areg IS     c.yeduc##c.age74 i.birthyr##c.lch71new [aw=wt] if year==2005 & young<., cluster(birthplnew) a(birthplnew)
eststo WSDlhwage10 : areg lhwage c.yeduc##c.age74 i.birthyr##c.lch71new [aw=wt] if year==2010 & young<., cluster(birthplnew) a(birthplnew)
eststo WSDlwage1314: areg lwage  c.yeduc##c.age74 i.birthyr##c.lch71new [aw=wt] if inlist(year,2011,2012,2013,2014) & young<., cluster(birthplnew) a(birthplnew)
...
cap drop lab
gen lab = ""
coefplot (RFprimaryc1wwty1, rename(1.young#c.ninnew=1995)) (RFprimaryc1wwty2, rename(1.young#c.ninnew=2005)) (RFprimaryc1wwty3, rename(1.young#c.ninnew=2010)) (RFprimaryc1wwty4, rename(1.young#c.ninnew=2011–14)), bylabel(Primary school completion) || ///
         (RFyeducc1wwty1, rename(1.young#c.ninnew=1995)) (RFyeducc1wwty2, rename(1.young#c.ninnew=2005)) (RFyeducc1wwty3, rename(1.young#c.ninnew=2010)) (RFyeducc1wwty4, rename(1.young#c.ninnew=2011–14)), bylabel(Years of schooling) || ///
         (WSDlhwage95E, rename(c.yeduc#c.age74=1995)) (WSDIS05, rename(c.yeduc#c.age74=2005)) (WSDlhwage10, rename(c.yeduc#c.age74=2010)) (WSDlwage1314, rename(c.yeduc#c.age74=2011–14)), bylabel(Wage scale dilation) || ///
         (RFpartc1wwty1, rename(1.young#c.ninnew=1995)) (RFpartc1wwty2, rename(1.young#c.ninnew=2005)) (RFpartc1wwty3, rename(1.young#c.ninnew=2010)) (RFpartc1wwty4, rename(1.young#c.ninnew=2011–14)), bylabel(Wage sector participation) || ///
         (RFlhwagec1wwty1, rename(1.young#c.ninnew=1995)) (RFISc1wwty2, rename(1.young#c.ninnew=2005)) (RFlhwagec1wwty3, rename(1.young#c.ninnew=2010)) (RFlwagec1wwty4, rename(1.young#c.ninnew=2011–14)), bylabel(Log earnings) ///
...
`graph'
graph export Public\Output\AgeDifferentiation.png, replace width(2680) height(1552)
restore
}
...
mata splinefit = `=_b[c.t1#c.nin]' * (24 :- (24::2)) + `=_b[c.t2#c.nin]' * (J(24-12,1,0) \ 12 :- (12::2))
mata st_numscalar("splineshift", mean(dots - splinefit))
local splinefn `=splineshift' 24 `=_b[c.t1#c.nin] * 12 + splineshift' 12  `=_b[c.t1#c.nin] * 22 + _b[c.t2#c.nin] * 10 + splineshift' 2

local kinkpt1  = splineshift + _b[c.t1#c.nin] * (24 - 12)
local kinkpt2  = splineshift + _b[c.t1#c.nin] * (24 -  2) + _b[c.t2#c.nin] * max(0, 12 - 2)
local kinkpt2b = splineshift + _b[c.t1#c.nin] * (24 -  2)
$graph || scatteri  `splinefn', recast(connected) lcolor(maroon) lwidth(medium) mstyle(p1) mcolor(maroon) msym(diamond) msize(small) ///
...
{
preserve
keep if inrange(age74, -14, 34)
gen t1 = 29 - age74
gen t2 = max(0, 12 - age74)  // post-trend divergence term

forvalues c=1/1 /*0/3*/ {  // control sets, from none to full
  local controls: word `=`c'+1' of "" lch71new "lch71new en71new" "lch71new en71new wsppc"

  forvalues y=1/5 {
    local years   : word `y' of 1995 2005 2010 2011,2012,2013,2014 2005,2010,2011,2012,2013,2014
    local yearname: word `y' of "Survey: Intercensal 1995" "Intercensal 2005" "Labor 2010" "Socioeconomic 2011–14" "Post-1995"
    local depvars : word `y' of "primary yeduc part lhwage" "primary yeduc part IS" "primary yeduc part lhwage" "primary yeduc part lwage" "primary yeduc part _lwage"
    local Ndepvars: word count `depvars'
    local age74minspline: word `y' of 2 -8 -8 -8 -8
    local age74minplot  : word `y' of 2 -8 -13 -13 -13

    foreach wt in 1 wt {
      local wtexp = cond("`wt'"=="1", "", "[pw=wt]")  // reghdfe can't handle weights that are not variables

      forvalues d=`Ndepvars'/`Ndepvars' {
        local depvar: word `d' of `depvars'
        local age74maxspline = 29 - 5*(`d'==`Ndepvars')
        local age74maxplot   = 34 - 5*(`d'==`Ndepvars')

        eststo RF`depvar'c`c'`wt'y`y'dummies: reghdfe `depvar' i(`=1974-`age74maxplot''/`=1974-`age74minplot'-1')bn.birthyr#c.ninnew `wtexp' if inrange(age74,`age74minplot',`age74maxplot') & inlist(year,`years'), a(birthplnew birthyr##c.(`controls') year) cluster(birthplnew)  // RF = reduced form
        mat bRF = e(b)
        coefplot, keep(*.birthyr#c.ninnew) omitted rename(([0-9]+)[ob]?.birthyr#c.ninnew = \1, regex) vertical at(_coef, transform(1974 - @)) xscale(reverse) msym(smcircle) msize(small) gen replace
        global graph `r(graph)'

        eststo RF`depvar'c`c'`wt'y`y'pwlspline: reghdfe `depvar' c.t?#c.ninnew `wtexp' if inrange(age74,`age74minspline',`age74maxspline') & inlist(year,`years'), a(birthplnew birthyr##c.(`controls') year) cluster(birthplnew)

        mata dots = st_matrix("bRF")'; dots[length(dots)] = 0  // dots in plot, including base level's 0
        mata dots = dots[`age74maxplot'- `age74maxspline'+1::`age74maxplot'- `age74minspline'+1]
        mata splinefit = `=_b[c.t1#c.ninnew]' * (`age74maxspline' :- (`age74maxspline'::`age74minspline')) + `=_b[c.t2#c.ninnew]' * (J(`age74maxspline'-12,1,0) \ 12 :- (12::`age74minspline'))
        mata st_numscalar("splineshift", mean(dots - splinefit))
        local splinefn `=splineshift                                                                                  '  `age74maxspline' ///
                       `=splineshift + _b[c.t1#c.ninnew] * (29 -              12 )                                        '         12  ///
                       `=splineshift + _b[c.t1#c.ninnew] * (29 - `age74minspline') + _b[c.t2#c.ninnew] * (12 - `age74minspline')'  `age74minspline' 
        ...
        $graph || scatteri 0 `age74minspline', mstyle(p1) msym(smcircle) msize(small) ///  // zero for base year
               || scatteri `splinefn', lcolor(maroon) lwidth(medium) mstyle(p1) msym(diamond) msize(small) mcolor(maroon) lpat(solid) recast(connected) ///
                  title(`=cond(`d'==1, `""`yearname'", size(3.5) span"', "")') `=cond(`y'==1, "fxsize(21)", "yscale(off)")' ylab(, format(%3.2f) nogrid) ///
                  xlab(`age74minplot' `age74minspline' 12 24 29 34, nogrid) ///
                  `=cond("`d'"=="`Ndepvars'", `"xtitle("")"', "xlab(, nolab notick nogrid) xscale(off fill)")' ///
               ...
               || scatteri 0 0, msymbol(none) xaxis(2) yaxis(2) xscale(axis(2) off) yscale(axis(2) off) ///  // fake plot to set up extra axes with range [-1,1] for placing text
                    text(.95 -.98 "`caption'", xaxis(2) yaxis(2) place(e) color(black))
      }
    }
  }
  foreach wt in 1 wt {
    graph combine RFprimary`wt'y1 RFprimary`wt'y2 RFprimary`wt'y3 RFprimary`wt'y4 RFprimary`wt'y5, cols(5) graphregion(margin(zero)) name(RFprimary, replace) imargin(zero) ycommon nodraw l1title(`:var label primary', size(small))
    graph combine RFyeduc`wt'y1   RFyeduc`wt'y2   RFyeduc`wt'y3   RFyeduc`wt'y4   RFyeduc`wt'y5  , cols(5) graphregion(margin(zero)) name(RFyeduc  , replace) imargin(zero) ycommon nodraw l1title(`:var label yeduc  ', size(small))
    graph combine RFpart`wt'y1    RFpart`wt'y2    RFpart`wt'y3    RFpart`wt'y4    RFpart`wt'y5   , cols(5) graphregion(margin(zero)) name(RFpart   , replace) imargin(zero) ycommon nodraw l1title(`:var label part   ', size(small))
    graph combine RFlhwage`wt'y1  RFIS`wt'y2      RFlhwage`wt'y3  RFlwage`wt'y4   RF_lwage`wt'y5 , cols(5) graphregion(margin(zero)) name(RFlwage  , replace) imargin(zero) ycommon nodraw l1title(`:var label lhwage ', size(small)) fysize(25)

    graph combine RFprimary RFyeduc RFpart RFlwage, cols(1) graphregion(margin(zero)) name(RFc`c'w`wt', replace) xsize(7.5) ysize(8.34) imargin(zero) b1title(Age in 1974, xoffset(4) size(vsmall))
    graph export Public\Output\RFc`c'w`wt'pwlspline.png, replace width(2000)
  }
}
restore
}
...
forvalues y=5/5 {
  ...
  local yearnamess: word `y' of 1995 2005 2010 "2013–14" "2005, 2010, 2013–14"
  ...
  forvalues d=1/3 {
    ...
    mata splinefit = `=_b[c.t1#c.ninnew]' * (24 :- (24::2)) + `=_b[c.t2#c.ninnew]' * (J(24 - 12,1,0) \ 12 :- (12::2))
    mata st_numscalar("splineshift", mean(dots - splinefit))
    local splinefn `=splineshift'                                                                24 ///
                    `=splineshift + _b[c.t1#c.ninnew] * (24 - 12)'                                12 ///
                    `=splineshift + _b[c.t1#c.ninnew] * (24 -  2) + _b[c.t2#c.ninnew] * (12 - 2)'  2 
    local kinkpt = splineshift + _b[c.t1#c.ninnew] * (24 - 12)
    test c.t2#c.ninnew
    local caption: display "{it:p} = " %4.2f r(p)
    $graph || scatteri 0 2, mstyle(p1) msym(square) msize(medium) mcolor(blue) /*mcolor("68 82 125")*/ ///  // zero for base year 
           || scatteri `splinefn', lcolor(maroon) lwidth(medium) mstyle(p1) msym(diamond) msize(small) mcolor(maroon) lpat(solid) recast(connected) ylab(, format(%3.2f) nogrid) graphregion(margin(zero)) ///
...
preserve
local age74max 24
keep if age74<=`age74max' // & age74>=2
gen t1 = `age74max' - age74  // spline components -1974 + birth year
gen t2 = max(0, 12 - age74)  // 12-year-olds in '74 couldn't benefit
...
forvalues c=1/1 {
  ...
  forvalues y=1/5 {
    local years   : word `y' of 1995 2005 2010 2011,2012,2013,2014 2005,2010,2011,2012,2013,2014
    local yearname: word `y' of "Survey: Intercensal 1995" "Intercensal 2005" "Labor 2010" "Socioeconomic 2011–14" "Post-1995"
    local depvars : word `y' of "primary yeduc part lwage lhwage" "primary yeduc part IS" "primary yeduc part lhwage" "primary yeduc part lwage" "primary yeduc part _lwage"
    local Ndepvars: word count `depvars'
    ...
    foreach wt in 1 wt {
      ...
      forvalues d=1/`Ndepvars' {
        ...
        eststo RF`depvar'c`c'`wt'y`y'qspline: reghdfe `depvar' c.t?#c.ninnew `wtexp' if inlist(year,`years'), absorb(birthplnew birthyr##c.(`controls') year) cluster(birthplnew)  // piecewise-linear fit
        ...
        eststo RF`depvar'c`c'`wt'y`y'poly: reghdfe `depvar' c.t1#c.ninnew c.t1#c.t1#c.ninnew `wtexp' if inlist(year,`years'), absorb(birthplnew birthyr##c.(`controls') year) cluster(birthplnew)  // p fit
        ...
        $graph || scatteri  `splinefn', lcolor(maroon) lwidth(medium) mstyle(p1) msym(diamond) msize(small) mcolor(maroon) lpat(solid) recast(connected) ///
               || function y = `polyfn', lcolor(blue  ) lwidth(medium) lpat(solid) range(2 24) ///
                  title(`=cond(`d'==1, `""`yearname'", size(medium)"', "")') `=cond(`y'==1, "fxsize(21)", "yscale(off)")' ylab(, format(%3.2f) nogrid) ///
                  `=cond("`d'"=="`Ndepvars'", `"xtitle("") xlab(2(2)24, nogrid)"', "xlab(, nolab notick nogrid) xscale(off fill)")' ///
                  graphregion(margin(zero)) ///
                  name(RF`depvar'`wt'y`y', replace) nodraw ///
               || scatteri 0 2, mstyle(p1) msym(smcircle) msize(small) /// // zero for base year
               || scatteri 0 0, msymbol(none) xaxis(2) yaxis(2) xscale(axis(2) off) yscale(axis(2) off) ///  // fake plot to set up extra axes with range [-1,1] for placing text
  ...
  foreach wt in 1 wt {
    ...
    graph combine RFpart`wt'y1    RFpart`wt'y2    RFpart`wt'y3    hole0           RFpart`wt'y5   , cols(5) graphregion(margin(zero)) name(RFpart   , replace) imargin(1 0 0 0) ycommon nodraw l1title(`:var label part'   , size(small))
    graph combine RFlhwage`wt'y1  RFIS`wt'y2      RFlhwage`wt'y3  RFlwage`wt'y4   RF_lwage`wt'y5 , cols(5) graphregion(margin(zero)) name(RFlhwage , replace) imargin(1 0 0 0) ycommon nodraw l1title(`:var label lhwage' , size(small))

    graph combine RFprimary RFyeduc RFpart RFlhwage, cols(1) graphregion(margin(zero)) name(RFc`c'w`wt'poly, replace) xsize(7.5) ysize(8.34) imargin(1 1 0 0) b1title(Age in 1974, xoffset(4) size(vsmall))
...
***
*** OLS & 2SLS without splines, as in original
***
{
preserve
keep if inrange(age74, 2, 24)
xi i.young|ninnew i.dum|ninnew i.birthyr*lch71new i.birthyr*en71new i.birthyr*wsppc i.year i.birthplnew
set seed 230498257
xtset birthpl
forvalues c=1/1 /*1/3*/ {  // ~original Table 7 regressions
  local controls: word `c' of birthyr##c.ch71 "birthyr##c.(ch71 en71)" "birthyr##c.(ch71 en71 wsppc)"
  forvalues d=1/3 {
    local depvar: word `d' of part lwage lhwage
    eststo OLS`depvar'yeducc`c'w1y1Orig  :    areg `depvar'  yeduc                  `controls' if year==1995, a(birthpl)
    eststo TSLS`depvar'yeducc`c'1w1y1Orig: xtivreg `depvar' (yeduc = 1.young#c.nin) `controls' if year==1995, fe small
    eststo TSLS`depvar'yeducc`c'2w1y1Orig: xtivreg `depvar' (yeduc =     dum#c.nin) `controls' if year==1995, fe small
  }
}
xtset birthplnew
forvalues c=1/1 /*1/3*/ {
  local controls _Iyear* _Ibirthyr_* `: word `c' of _IbirXlch_* "_IbirXlch_* _IbirXen7_*" "_IbirXlch_* _IbirXen7_* _IbirXwsp_*" '
  foreach edvar in yeduc primary {
    forvalues y=1/5 {
      local years   : word `y' of 1995 2005 2010 2011,2012,2013,2014 2005,2010,2011,2012,2013,2014
      local depvars : word `y' of "part lwage lhwage" "part IS" "part lhwage" "part lwage" "part _lwage" 
      local Ndepvars: word count `depvars'
      local graphs
      forvalues d=1/`Ndepvars' {
        local depvar: word `d' of `depvars'
        foreach wt in 1 wt {
          ...
          forvalues i=1/2 {
            local insts: word `i' of _IyouXninne_1 _IdumXnin_*
            ...
            boottest `edvar', gridmin(-.5) gridmax(.5) format(%4.2f) ///
                     graphopt(xlab(-.5(.1).5) ylab(.05 .2(.2)1, nogrid) xline(0) nodraw ///
        ...
        esttab OLS`depvar'`edvar'c`c'w*y`y'* TSLS`depvar'`edvar'c`c'1w*y`y'* TSLS`depvar'`edvar'c`c'2w*y`y'* ///
      ...
      graph export Public\Output\TSLS`edvar'c`c'y`y'.png, replace width(2000)
    }
  }
}
restore
...
xi i.young|ninnew i.dum|ninnew i.birthyr*lch71new i.birthyr*en71new i.birthyr*wsppc i.year  // xtivreg2 doesn't take factor vars, but is faster than ivreg2...i.birthplnew and does weak inst tests
set seed 230498257
...
forvalues c=0/0 /*0/3*/ {
  ...
  foreach edvar in yeduc primary {
    forvalues y=1/5 {
      local years  : word `y' of 1995 2005 2010 2011,2012,2013,2014 2005,2010,2011,2012,2013,2014
      local depvars : word `y' of "part lhwage" "part IS" "part lhwage" "part lwage" _lwage
      local Ndepvars: word count `depvars'
      local graphs
      forvalues d=1/`Ndepvars' {
        local depvar: word `d' of `depvars'
        foreach wt in 1 wt {
          forvalues i=1/2 {
            ...
            eststo TSLSPWL`depvar'`edvar'c`c'`i'`wt'y`y': xtivreg2 `depvar' (`edvar' = t2_nin) `timecontrols' `controls' _Iyear* [pw=`wt'] if inlist(year,`years'), cluster(birthplnew) partial(`controls') small fe

            weakivtest
            estadd scalar F_eff = r(F_eff)

            boottest `edvar', /*ar reps(99999)*/ gridmin(-.8) gridmax(1.1) format(%4.2f) ///
        ...
        esttab TSLSPWL`depvar'`edvar'c`c'21y`y' TSLSPWL`depvar'`edvar'c`c'2wty`y' TSLSPWL`depvar'`edvar'c`c'11y`y' TSLSPWL`depvar'`edvar'c`c'1wty`y' ///
               ...
               stat(CIstr F_eff N, labels("Bootstrap CI" "OP F" Observations) fmt(%~1s %4.2f %7.0fc)) fonttbl(\f0\fnil Cambria;)
...
xi i.birthyr|lch71new i.birthyr|en71new i.birthyr|wsppc i.year
set seed 30948573
forvalues y=5/5 {
  local years : word `y' of 1995 2005 2010 2011,2012,2013,2014 2005,2010,2011,2012,2013,2014
  local depvar: word `y' of lhwage IS lhwage lwage _lwage
  eststo cic`y': cic `depvar' _IbirXch7* _Iyear* [pw=wt] if inlist(year,`years'), group(recpnew) time(young) reps(`=cond(`y'<5,1000,100)')
...
coefplot cic1 cic2 cic3 cic4 cic5, scheme(plottig) ylabel(1 "10" 2 "20" 3 "30" 4 "40" 5 "50" 6 "60" 7 "70" 8 "80" 9 "90") ytitle(Percentile) ///
  ...
  legend(rowgap(zero) lab(21 "Log hourly wage, 1995") lab(42 "Imputed log hourly wage, 2005") lab(63 "Log hourly wage, 2010") lab(84 "Log typical monthly earnings, 2011–14") lab(105 "Log earnings, post-1995") pos(4) region(/*fcolor(gs14%100) lstyle(none)*/ margin(zero)) bmargin(1 0 0 0) size(7pt)) ///
